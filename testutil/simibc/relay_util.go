...
func augmentHeader(sender, receiver *ibctesting.TestChain, clientID string, header *ibctmtypes.Header) error {
	// In IBC v10, LatestHeight is a field on the tendermint ClientState
	clientState := receiver.GetClientState(clientID).(*ibctmtypes.ClientState)
	...
	// IBC v10 change: LastHeader renamed to LatestCommittedHeader
	if trustedHeight == sender.LatestCommittedHeader.GetHeight() {
		...
		// NextValidatorsHash
		// IBC v10 changes:
		// 1. Use TrustedValidators map instead of GetValsAtHeight method
		// 2. Map key is uint64, not int64

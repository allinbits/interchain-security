...
func (k Keeper) AppendConsumerAddrsToPrune(ctx sdk.Context, chainID string, pruneTs time.Time, consumerAddr types.ConsumerConsAddress) {
	store := ctx.KVStore(k.storeKey)
...
// to prune stored under timestaps ts.
...
func (k Keeper) ConsumeConsumerAddrsToPrune(ctx sdk.Context, chainID string, ts time.Time) (consumerAddrsToPrune types.AddressList) {
	store := ctx.KVStore(k.storeKey)
	consumerAddrsToPruneKeyPrefix := types.ConsumerAddrsToPruneV2BytePrefix
...
func (k Keeper) DeleteConsumerAddrsToPruneV2(ctx sdk.Context, chainID string, pruneTs time.Time) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) DeleteKeyAssignments(ctx sdk.Context, chainID string) {
	...
	for _, consumerAddrsToPrune := range k.GetAllConsumerAddrsToPrune(ctx, chainID) {
		k.DeleteConsumerAddrsToPruneV2(ctx, chainID, consumerAddrsToPrune.PruneTs)
	}
}
